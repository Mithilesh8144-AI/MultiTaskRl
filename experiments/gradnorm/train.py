"""
GradNorm DQN Training Script

Train a multi-task DQN agent using GradNorm loss balancing.
GradNorm dynamically balances task losses via learnable weights.

Key differences from Shared DQN:
    - Uses PerTaskReplayBuffer (separate buffer per task)
    - Learnable task weights adjusted via GradNorm algorithm
    - Tracks and logs weight evolution over training
"""
...
from agents.pcgrad import PerTaskReplayBuffer  # Reuse per-task buffer
from experiments.gradnorm.config import get_config


def save_gradnorm_metrics(all_episodes, eval_history, best_rewards, config,
                          total_env_steps, total_gradient_updates, weight_evolution,
                          num_params):
    """
    Save training metrics including GradNorm-specific weight evolution.
    ...
    Args:
        ...
        weight_evolution: Dict of task weight histories
        num_params: Number of model parameters
    """
    output_name = config.get('output_dir', 'gradnorm')
    output_dir = project_root / 'results' / output_name / 'logs'
...
def train():
    """Train GradNorm DQN on all 3 tasks simultaneously."""
    # Load configuration
    config = get_config()
    print(f"\n{'='*80}")
    print(f"GRADNORM DQN - MULTI-TASK TRAINING")
    print(f"{'='*80}")
    ...
    output_name = config.get('output_dir', 'gradnorm')
    output_base = project_root / 'results' / output_name
    ...
    # Create per-task replay buffers (same as PCGrad)
    ...
    print(f"\nStarting GradNorm multi-task training...")
    ...
    for episode in range(total_episodes):
        ...
        while not (done or truncated):
            ...
            if replay_buffer.can_sample_all(config['batch_size']):
                # Sample batches from all tasks
                task_batches = [
                    replay_buffer.sample(i, config['batch_size'])
                    for i in range(3)
                ]

                loss, gradnorm_stats = agent.update(task_batches)
                episode_losses.append(loss)
                episode_weights.append(gradnorm_stats['weights'])
        ...
        current_weights = agent.get_current_weights()
        recent_weights.append(current_weights)
        if len(recent_weights) > 100:
            recent_weights.pop(0)

        # Store episode data (including weight info)
        episode_data = {
            'episode': episode,
            'task': task_name,
            'task_id': task_id,
        ...
        if episode % config['eval_freq'] == 0 and episode > 0:
            ...
            # Log eval results and weight stats
            pbar.write(f"\nEpisode {episode} Evaluation:")
            for task, reward in eval_results.items():
                pbar.write(f"   {task.capitalize()}: {reward:7.2f}")
            pbar.write(f"   Task Weights: STD={current_weights[0]:.3f}, "
                      f"WIN={current_weights[1]:.3f}, HVY={current_weights[2]:.3f}")
        ...
        pbar.set_postfix({
            ...
            'W': f'{current_weights[0]:.1f}/{current_weights[1]:.1f}/{current_weights[2]:.1f}'
        })
    ...
    # Get final weight evolution
    weight_evolution = agent.get_weight_evolution()
    ...
    print(f"\nFinal Task Weights:")
    final_weights = agent.get_current_weights()
    task_names = ['Standard', 'Windy', 'Heavy']
    for name, w in zip(task_names, final_weights):
        print(f"  {name}: {w:.4f}")
    print(f"\nGradNorm Statistics:")
    print(f"  Alpha: {config['gradnorm_alpha']}")
    if weight_evolution:
        print(f"  Weight range (Standard): [{min(weight_evolution['standard']):.3f}, {max(weight_evolution['standard']):.3f}]")
        print(f"  Weight range (Windy): [{min(weight_evolution['windy']):.3f}, {max(weight_evolution['windy']):.3f}]")
        print(f"  Weight range (Heavy): [{min(weight_evolution['heavy']):.3f}, {max(weight_evolution['heavy']):.3f}]")
    print(f"\nSample Efficiency:")
